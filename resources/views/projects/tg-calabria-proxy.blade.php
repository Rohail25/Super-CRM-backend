...
<head>
    ...
    <title>TG Calabria Project</title>
    <style>
...
<body>
    <div class="container">
        <div class="loading">
            ...
            <p class="message">Loading TG Calabria project...</p>
        </div>
    </div>

    <script>
        // Pass data to parent window so frontend can display it
        console.log('TG Calabria proxy page loaded for user: {{ $user->email }}');
        
        // Notify parent that we're ready (if in iframe)
        if (window.parent !== window) {
            window.parent.postMessage({
                type: 'tg-calabria-proxy-ready',
                user: {
                    id: {{ $user->id }},
                    email: '{{ $user->email }}',
                    name: '{{ $user->name }}'
